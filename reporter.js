...
class Reporter {
   ...
   * @param {Boolean} options.screenshotOnFail - Save screenshot on failed test
   * @param {Boolean} options.screenshotOnSuccess - Save screenshot on succesful test
   */
  constructor(options) {
  ...
  specDone(result) {
    ...
    result.duration =
      new Date(result.stopped) - new Date(this.starts[result.id])

    // suspectLine
    result.failedExpectations.forEach((failure) => {
      failure.hasSuspectLine = failure.stack.split('\n').some(function (line) {
        let match =
          line.indexOf('Error:') === -1 && line.indexOf('node_modules') === -1
    ...
    JSONResult.pending = ['pending', 'disabled', 'excluded'].includes(
      result.status
    )
    ...
    if (JSONResult.passed) {
      JSONResult.message =
        (result.passedExpectations[0] || {}).message || 'Passed'
      ...
      if (result.failedExpectations[0].message) {
        JSONResult.message = result.failedExpectations.map(
      ...
      if (result.failedExpectations[0].stack) {
        JSONResult.trace = result.failedExpectations.map(
          (result) => result.stack
        )
      } else {
  ...
  writeFile() {
    ...
    let results = fileContents.replace(
      "'<Results Replacement>'",
